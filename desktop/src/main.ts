...
function loadConfig(): AssistantConfig {
  ...
  return {
    ...
    window: { width: 480, height: 600, alwaysOnTop: true, opacity: 0.92, transparent: true, frameless: true },
    ...
    startup: { autoStart: true, minimizeToTray: true, startMinimized: true },
  };
}
...
function createWindow(): void {
  const { width, height, alwaysOnTop, opacity, transparent, frameless } = config.window;
...
function createTray(): void {
  // Create tray icon — try loading from file, fallback to empty
...
function registerShortcuts(): void {
  ...
  globalShortcut.register(config.shortcuts.activate, () => {
    toggleWindow();
    if (mainWindow) {
  ...
  globalShortcut.register(config.shortcuts.toggleWindow, () => {
    toggleWindow();
  });
}
...
async function runStartupSequence(): Promise<void> {
  ...
  if (mainWindow) {
    mainWindow.webContents.send("jarvis:startupProgress", {
      step: "connecting",
      status: "checking",
...
app.whenReady().then(async () => {
  try {
    // 1. Ask for startup permission on first launch
    await askStartupPermission();

    // 2. Configure auto startup in Windows login items
    configureAutoStartup();

    // 3. Spawn the Python backend as a hidden child process
    console.log("[Jarvis] Starting Python backend...");
    await trySpawnBackend();

    // 4. Create window, tray, and shortcuts immediately (shows startup splash)
    createWindow();
    // Window is created but will show startup progress via IPC
    createTray();
    registerShortcuts();
    setupIPC();
    startBackendMonitoring();

    // 5. Run the full startup sequence with diagnostics and greeting
    // This handles: wait for backend health → run diagnostics → speak greeting
    runStartupSequence();
  } catch (err) {
